...
@section('title', 'Update Donar')
@section('main')

    <div class="relative overflow-x-auto shadow-md sm:rounded-lg py-16 px-10 m-4">
        <form action="{{ route('donars.update',$donar->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="bn_name" name="bn_name"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
                    value="{{ old('bn_name',$donar->bn_name) }}">
            </div>
            <div class="mb-6">
                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                <input type="email" id="email" name="email"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
                    value="{{ old('email',$donar->email) }}">
            </div>
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="last_donation_date" name="last_donation_date"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
                    value="{{ old('last_donation_date',$donar->last_donation_date) }}">
            </div>
            <div class="mb-6">
                <label for="address" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Address</label>
                <textarea id="address" type="textarea" name="address" rows="2" minHeight="500"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">{{ $donar->address }}</textarea>
            </div>
            <div class="mb-6">
                ...
                <select id="area" name="area_id"
                    ...
                    @foreach ($areas as $area)
                        <option value="{{ $area->id }}" @if ($area->id == $donar->area_id)
                            selected
            ...
            <div class="mb-6">
                ...
                <select id="gender" name="gender"
                    ...
                    <option value="male" @if ($donar->gender == 'male')
                        selected
                    @endif>Male</option>
                    <option value="female" @if ($donar->gender == 'female')
                        selected
                    @endif>Female</option>
                </select>
                 @error('gender')
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                    <option value="" disabled> Select</option>
                    <option value="pending" @if ($donar->status == 'pending')
                        selected
                    @endif>Pending</option>
                    <option value="approved" @if ($donar->status == 'approved')
                        selected
            ...
            <div class="mb-6">
                ...
                <select id="blood_group" name="blood_group"
                    ...
                    <option value="{{ $data }}" @if ($donar->blood_group == $data)
                        selected
                    @endif>{{ $data }}</option>
                    @endforeach
                </select>
